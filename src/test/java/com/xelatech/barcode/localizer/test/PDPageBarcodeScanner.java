...
public class PDPageBarcodeScanner {
	...
	private static Map<DecodeHintType, Object> decodeHintTypes = new Hashtable<DecodeHintType, Object>();
	static {
	...
	private static int subImageCounter = 1;
	...
	public PDPageBarcodeScanner(final PDPage pdPage, final int pageNumber, final int maximumBlankPixelDelimiterCount) {
		this.pdPage = pdPage;
	...
	public void displayResults() {
		for (final Result result : this.results.values()) {
			System.out.println("\nPage = " + (this.pageNumber + 1) + ", barcode format = " + result.getBarcodeFormat()
	 ...
	 * @throws IOException
	 */
	public static List<Rectangle> getAllAreaByColor(final BufferedImage in, final BufferedImage out, final int redColor,
			final int greenColor, final int blueColor, final int maximumBlankPixelDelimiterCount, final boolean debug)
			throws IOException {
	...
	public static List<Rectangle> getAreaList(final BufferedImage image, final int maximumBlankPixelDelimiterCount)
			throws IOException {
		// Use 'out' image for debug purposes.
		BufferedImage out = PDPageBarcodeScanner.copyImage(image, BufferedImage.TYPE_INT_ARGB);

		List<Rectangle> areas = PDPageBarcodeScanner.getAllAreaByColor(image, out/* null */, 0, 0, 0,
				maximumBlankPixelDelimiterCount, false);

		ImageIO.write(out, "png", new File(
				PDPageBarcodeScanner.WS_PATH + "/test_4_img_" + PDPageBarcodeScanner.imageCounter++ + "." + "png"));
	...
	public void extractBarcodeArrayByAreas(final BufferedImage image, final int maximumBlankPixelDelimiterCount)
			throws IOException {
		final BufferedImage blackAndWhiteImage = PDPageBarcodeScanner.getThresholdImage(image);

		// ImageIO.write(blackAndWhiteImage, "png", new
		// File(PdPageBarcodeScanner.WS_PATH + "/test_3_bw-img.png"));

		final List<Rectangle> areaList = PDPageBarcodeScanner.getAreaList(blackAndWhiteImage,
				maximumBlankPixelDelimiterCount);
		...
		for (final Rectangle rectangle : areaList) {
			...
			final BufferedImage croppedImage = PDPageBarcodeScanner.copyImage(
			...
			this.addResults(PDPageBarcodeScanner.extractBarcode(croppedImage));
			this.addResults(PDPageBarcodeScanner.extractBarcode(
					PDPageBarcodeScanner.rotate90ToLeftImage(croppedImage, BufferedImage.TYPE_INT_ARGB)));
			this.addResults(PDPageBarcodeScanner.extractBarcode(
					PDPageBarcodeScanner.rotate90ToRightImage(croppedImage, BufferedImage.TYPE_INT_ARGB)));
			this.addResults(PDPageBarcodeScanner
					.extractBarcode(PDPageBarcodeScanner.rotate180Image(croppedImage, BufferedImage.TYPE_INT_ARGB)));
			...
			this.addResults(this.extractMultipleBarcode(
					PDPageBarcodeScanner.rotate90ToLeftImage(croppedImage, BufferedImage.TYPE_INT_ARGB)));
			this.addResults(this.extractMultipleBarcode(
					PDPageBarcodeScanner.rotate90ToRightImage(croppedImage, BufferedImage.TYPE_INT_ARGB)));
			this.addResults(this.extractMultipleBarcode(
					PDPageBarcodeScanner.rotate180Image(croppedImage, BufferedImage.TYPE_INT_ARGB)));
		}
	}
	...
	public Result[] extractMultipleBarcode(final BufferedImage bufferedImage) {
		...
		try {
			...
			results = this.reader.decodeMultiple(bitmap, PDPageBarcodeScanner.decodeHintTypes);
	...
	public static Result[] extractBarcode(final BufferedImage bufferedImage) {
		Result[] results = null;
		...
		try {
			final Result result = new MultiFormatReader().decode(bitmap, PDPageBarcodeScanner.decodeHintTypes);
			results = new Result[1];
			results[0] = result;
		} catch (final Exception e) {
			try {
				final Result result = new Code128Reader().decode(bitmap, PDPageBarcodeScanner.decodeHintTypes);
				results = new Result[1];
				results[0] = result;
	 ...
	 */
	public static BufferedImage rotate180Image(final BufferedImage inputImage, final int imageType) {
	 ...
	 * @see http://forum.codecall.net/topic/69182-java-image-rotation/
	 */
	public static BufferedImage rotate90ToRightImage(final BufferedImage inputImage, final int imageType) {
		final int width = inputImage.getWidth();
		final int height = inputImage.getHeight();
	 ...
	 * @see http://forum.codecall.net/topic/69182-java-image-rotation/
	 */
	public static BufferedImage rotate90ToLeftImage(final BufferedImage inputImage, final int imageType) {
		final int width = inputImage.getWidth();
		final int height = inputImage.getHeight();
		final BufferedImage returnImage = new BufferedImage(height, width, imageType);
	...
	public static BufferedImage getBlackAndWhiteImage(final BufferedImage image) throws IOException {
	...
	public static BufferedImage getThresholdImage(final BufferedImage image) throws IOException {
		final float saturationMin = 0.10f;
		final float brightnessMin = 0.80f;
		final BufferedImage thresholdImage = PDPageBarcodeScanner.copyImage(image, BufferedImage.TYPE_INT_ARGB);

		PDPageBarcodeScanner.computeBlackAndWhite(image, thresholdImage, saturationMin, brightnessMin);

		return thresholdImage;
	}

	public static void computeBlackAndWhite(final BufferedImage in, final BufferedImage out, final float saturationMin,
	...
	public static final BufferedImage copyImage(final BufferedImage bi, final int type) {
